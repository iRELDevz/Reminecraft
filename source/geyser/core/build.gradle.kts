...
}.orElse("https://github.com/GeyserMC/Geyser")
...
val finalVersion = projectVersionProvider.zip(gitVersion) { projVer, gitVer ->
    "$projVer ($gitVer)"
}.orElse("${project.version} (git-DEV-0000000)")
...
sourceSets {
    main {
        blossom {
            javaSources {
                property("version", finalVersion.getOrElse(project.version.toString()))
                property("gitVersion", gitVersion.getOrElse("git-DEV-0000000"))
                property("buildNumber", buildNumber.getOrElse("0"))
                property("branch", gitBranch.getOrElse("DEV"))
                property("commit", gitCommitName.getOrElse("0".repeat(40)))
                property("repository", gitRepositoryUrl.getOrElse("https://github.com/GeyserMC/Geyser"))
                property("devVersion", gitRepositoryIsDev.getOrElse(true).toString())
            }
        }
    }
}
